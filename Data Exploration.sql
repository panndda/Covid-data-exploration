...
--Total Cases
select 	cast(max(total_cases) as bigint)
from covid_deaths

--Total_Deaths
select 	cast(max(total_deaths) as bigint)
from covid_deaths
...
Select country, population, cast(max(total_deaths)as bigint) AS total_death, max(total_deaths/population)*100 as death_rate
from covid_deaths
where continent is not null
group by country,population
...
Select country, cast(max(total_deaths) as integer) as total_death
from covid_deaths
...
select country, total_death,rank() over(order by total_death desc)
from cont;
...
--Dates when each country started to administer vaccinations and their running sum through the years with WINDOWS FUNCTION
...
--Countries that are yet to be fully vaccinated
select  (round(cast(count(*)as numeric)/cast((select count(distinct country) from covid_deaths where continent is not null) as numeric),2))*100
from popvac1
